...
class EntanglementRegularizer(nn.Module):
    ...
    def forward(self, y):
        ...
        # FIX: Precision Shield (Force float32 for sensitive entropy math)
        # Add stronger epsilon (1e-7) to trace to prevent division by zero
        y_flat = y.view(B, C, -1).to(torch.float32)
